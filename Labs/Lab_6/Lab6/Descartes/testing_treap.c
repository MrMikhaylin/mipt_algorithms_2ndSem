...
void run_treap_tests (const char* TestsPath, const char* ResultsPathInsertion, const char* ResultsPathDeleting, size_t from, size_t to, size_t step)
{
    ...
    {       
        for (size_t j = 0; j < BLOCK_LENGHT; j++)                         // перебор по каждой пятерке
        {
            ...
            for (int k = 0; k < i; k++)
            {
                ...
                tree = insert_number (tree, nums[k], rand ());
            }
...
int main()
{
    run_treap_tests ("../tests/arrays/", "../results/insertion/Treap", "../results/deleting/Treap", 100000, 1000000, 100000);
